...
export class HubSpotSyncService {
  ...
  /**
   * Upsert LinkedIn messages to HubSpot Communications
   */
  async upsertLinkedInMessages(
    ...
    try {
      ...
      const handle = this.extractLinkedInHandle(contactPerson.profileUrl);
      if (!handle) throw new Error("Invalid LinkedIn profile URL");

      const contact = await this.findContactByProfileUrl(handle);
      if (!contact) {
        throw new Error(
      ...
      const existingComms = await this.getExistingCommunications(contactId);

      // Create lookup map by date only - FIX: Parse ISO timestamp correctly
      const existingByDate = new Map<string, string>();
      for (const comm of existingComms) {
        // HubSpot returns ISO string, extract date directly
      ...
      return {
        success: true,
        threadId: this.generateThreadId(conversationKey),
        contactId,
        synced: syncedCount,
        skipped: 0,
        messages: results,
  ...
  private groupMessagesByDate(
    ...
    for (const message of messages) {
      const date = message.sentAt.split("T")[0]; // Extract YYYY-MM-DD
      logger.info(
    ...
    for (const [date, dayMessages] of grouped) {
      dayMessages.sort(
  ...
  /**
   * Create formatted body for daily messages (email-style format)
   */
  ...
  /**
   * Identify contact person (distance !== "SELF")
   */
  ...
  /**
   * Get existing communications for contact
   */
  ...
  /**
   * Upsert single message as HubSpot communication
   */
  private async upsertDailyCommunication(
    ...
    // FIX: Send ISO string at midnight UTC, not milliseconds
  ...
  /**
   * Generate thread ID from conversation key
   */
  private generateThreadId(conversationKey: string): string {
  ...
  /**
   * Check message sync status for a contact
   */
  // async checkMessageSyncStatus(
  //   contactProfileUrl: string,
  // ): Promise<CheckMessagesResponse> {
  //   logger.info(
  //     `[Messages Check] Checking sync status for: ${contactProfileUrl}`,
  //   );

  //   try {
  //     // Step 1: Extract LinkedIn handle
  //     const handle = this.extractLinkedInHandle(contactProfileUrl);
  //     if (!handle) {
  //       logger.error(
  //         `[Messages Check] Invalid LinkedIn URL: ${contactProfileUrl}`,
  //       );
  //       throw new Error("Invalid LinkedIn profile URL");
  //     }
  //     logger.debug(`[Messages Check] LinkedIn handle: ${handle}`);

  //     // Step 2: Find contact in HubSpot
  //     logger.info(`[Messages Check] Searching for contact...`);
  //     const contact = await this.findContactByProfileUrl(handle);

  //     if (!contact) {
  //       logger.info(`[Messages Check] Contact not found in HubSpot`);
  //       return {
  //         contactExists: false,
  ...
  //     const contactId = contact.id;
  //     logger.info(`[Messages Check] Contact found: ${contactId}`);

  //     // Step 3: Get all communications for contact
  //     logger.info(`[Messages Check] Fetching communications...`);
  //     const communications = await this.getExistingCommunications(contactId);
  //     logger.debug(
  //       `[Messages Check] Found ${communications.length} communications`,
  //     );

  //     if (communications.length === 0) {
  //       logger.info(`[Messages Check] No messages synced yet`);
  //       return {
  //         contactExists: true,
  //         contactId,
  //         hasSyncedMessages: false,
  ...
  //     // Step 4: Find latest message timestamp
  //     const timestamps = communications
  //       .map((comm) => {
  //         // HubSpot returns timestamp as string, could be in milliseconds or ISO format
  //         const ts = comm.timestamp;

  //         // Try parsing as number (milliseconds)
  //         const numTs = parseInt(ts);
  //         if (!isNaN(numTs) && numTs > 1000000000000) {
  //           // Valid millisecond timestamp (after year 2001)
  //           return numTs;
  //         }

  //         // Try parsing as ISO date string
  //         const dateTs = new Date(ts).getTime();
  //         if (!isNaN(dateTs)) {
  //           return dateTs;
  //         }
  ...
  //       logger.warn(`[Messages Check] No valid timestamps found`);
  //       return {
  //         contactExists: true,
  //         contactId,
  //         hasSyncedMessages: false,
  ...
  //     const istDate = new Date(latestTimestamp);
  //     istDate.setHours(istDate.getHours() + 5);
  //     istDate.setMinutes(istDate.getMinutes() + 30);
  //     const latestMessageTimestamp = istDate.toISOString();

  //     logger.warn(`[Messages Check] Latest message: ${latestMessageTimestamp}`);
  //     logger.warn(`[Messages Check] Total messages: ${communications.length}`);
  ...
  //     };
  //   } catch (error: any) {
  //     logger.error(`[Messages Check] Failed: ${error.message}`);
  //     throw error;
  //   }
  // }
}
